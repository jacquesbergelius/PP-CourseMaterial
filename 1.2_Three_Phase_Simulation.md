...
It is quite easy to implement time-slice simulator in Java. The number of current time-slice being processed can be kept in a variable. Time delays (time between customer arrivals and operator service times) can be implemented using counter variables.
```Java
...
```
> Note that the previous simulator should have been implemented using a class for the operator. In this way we can implement only one operator and then instantiate the class to create two operator objects.
